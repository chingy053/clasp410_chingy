...
for idx, ii in enumerate(list_pbare):
    # Run the model
    forest_3, full_burn_time_33[idx] = forest_fire(isize=100, jsize=100, nstep=40, case='disease' ,pspread=0.65, pignite=0.05, pbare=ii, pfetal=0.05)
    ...
    ax3.plot(perc_healthy,color=cmap(ii))
    ax4.plot(perc_sick,color=cmap(ii))
ax5.plot(list_pspread,full_burn_time_33,color='k',linewidth=2)
...
for idx, ii in enumerate(list_pfetal):
    ...
    ax3.plot(perc_healthy,color=cmap(ii))
    ax4.plot(perc_sick,color=cmap(ii))
ax5.plot(list_pspread,full_burn_time_333,color='k',linewidth=2)
