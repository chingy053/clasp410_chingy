...
def gen_grid(npoints=18):
    '''
    Create a evenly spaced latitudinal grid with 'npoints' cell center.
    Grid will always run from zero to 180 as the edges of the grid. This means that the first grid point will be 'dLat/2' from 0 degrees and the last point will be '180 - dLat/2'.
...
def snowball_earth(nlat=18, tfinal=10000, dt=1.0,lam=100,emiss=1.0,init_cond=temp_warm, apply_spherecorr=False, apply_sol=False, albice = 0.6, albgnd=0.3, apply_insol=False, solar=1370., gamma=1.):
    '''
    Solve the snowball Earth problem.
    ...
    emiss: float, defaults to 1.0
        Emissivity of the Earth from 0 to 1.
    init_cond: function, float, or array
        Set the initial condition of the simulation. If a function is given, it must take latitudes as input and return temperature as a function of lat. Otherwise, the given values are used as is.
    apply_spherecorr: bool, defaults to False
        Apply spherical correction term.
    apply_insol: bool, defaults to False
        Apply insolation terms.
    solar: float, defaults to 1370 W/m2
        Set level of solar forcing.
    albice, albgnd: floats, default to 0.6 and 0.3
        Albedo of the ice and and ground.
    gamma: float, defaults to 1
        A solar multiplier factor applied to the solar insolation.
    ...
    # Set timstep to seconds
    dt = dt*365*24*3600

    # Create insolutions
    insol = gamma * insolation(solar, lats) 

    # Cretae temp array: set our initial condition
    Temp = np.zeros(nlat)
